...
(function() {
  ...
  const I18N = {
    'zh-CN': {
      ...
      updateCheckGrp: '更新检查',
      updateSourceLabel: '更新源',
      sourceGithub: 'GitHub 官方（需代理）',
      sourceGhproxy: 'ghproxy 镜像（推荐国内）',
      sourceGhfast: 'ghfast 镜像（备用）',
      connectingGithub: '正在连接 GitHub...',
      githubSlowTip: '网络连接较慢，请耐心等待（国内访问 GitHub 较慢）...',
      githubTimeout: '连接超时，网络不通畅（可能被墙或DNS污染）。建议开启代理/VPN后重试。',
      updateFailed: '更新检查失败，可能是网络问题（GitHub 在国内访问不稳定），可稍后重试或手动下载新版本',
    ...
    'en-US': {
      ...
      sourceGithub: 'GitHub Official (requires proxy)',
      sourceGhproxy: 'ghproxy Mirror (recommended for China)',
      sourceGhfast: 'ghfast Mirror (backup)',
      connectingGithub: 'Connecting to GitHub... (may be slow in China)',
      githubSlowTip: 'Connection is slow, please wait... (GitHub access is slow from China)',
      githubTimeout: 'Connection timeout — network unreachable (may be blocked/DNS issue). Please enable proxy/VPN and try again.',
  ...
  function bindEvents() {
    ...
    if (btnCheckUpdateAbout) {
      btnCheckUpdateAbout.addEventListener('click', function() {
        ...
        var selectedSource = srcSelect ? srcSelect.value : 'github';
        ...
        if (statusText) statusText.textContent = t('connectingGithub');
        // 重置状态以便重新接收事件
        _checkingForUpdate = true;
        // 10秒后提示用户网络可能较慢（不中断检查，只是提示）
        clearTimeout(_updateTimeoutTimer);
        _updateTimeoutTimer = setTimeout(function() {
          if (_checkingForUpdate && statusText && !statusText.textContent.includes(t('githubSlowTip'))) {
            statusText.textContent = t('githubSlowTip');
          }
        }, 10000);
        // 25秒后判定超时，自动报错并终止等待
        var _abortTimer = setTimeout(function() {
          if (_checkingForUpdate) {
            _checkingForUpdate = false;
            btnCheckUpdateAbout.disabled = false;
            btnCheckUpdateAbout.textContent = t('updateCheck');
            if (statusText) statusText.textContent = '';
            showUpdateResultDialog(false, null, t('githubTimeout'));
          }
        }, 25000);
        window.electronAPI.updaterCheck().finally(function() {
...
`;
  ...
  function handleUpdateEvent(data) {
    switch (data.state) {
      ...
      case 'not-available':
        ...
        if (_checkingForUpdate) {
          _checkingForUpdate = false;
          showUpdateResultDialog(true, null, null);
        }
        break;
      case 'downloading':
      ...
      case 'error':
        if (data.message && data.message.indexOf('404') === -1) {
          showToast(t('updateErrorPrefix') + data.message, 3000);
        }
        break;
    }
  }
  ...
  // 显示更新结果弹窗（已是最新 / 出错）
  function showUpdateResultDialog(isLatest, info, customMsg) {
    if (_updateModal) return;
    _updateModal = document.createElement('div');
    _updateModal.className = 'about-modal-overlay';
    var title = isLatest ? t('latestVersion') : (customMsg || t('updateFailed'));
    var bodyHtml = '';
    if (isLatest) {
      bodyHtml = '<p style="text-align:center;font-size:14px;color:var(--text-secondary);margin:12px 0;">' + t('latestBody') + ' v9.8.2</p>';
    } else {
      bodyHtml = '<p style="text-align:center;font-size:14px;color:var(--text-secondary);margin:12px 0;">' + escapeHtml(customMsg || '') + '</p>';
    }
    bodyHtml += '<div style="display:flex;justify-content:center;margin-top:16px;"><button class="action-btn primary-btn" id="update-result-ok" style="padding:6px 28px;">' + t('confirm') + '</button></div>';

    _updateModal.innerHTML = ''
      + '<div class="about-modal-box" style="max-width:340px;">'
      + '  <div class="about-modal-header"><span>' + title + '</span><button class="about-modal-close" id="update-result-close">✕</button></div>'
      + '  <div class="about-modal-body">' + bodyHtml + '</div>'
      + '</div>';

    document.body.appendChild(_updateModal);
    _updateModal.querySelector('#update-result-close').addEventListener('click', closeUpdateModal);
    _updateModal.querySelector('#update-result-ok').addEventListener('click', closeUpdateModal);
  ...
  function checkAndShowChangelog() {
    try {
      ...
      // 获取当前版本
      var currentVersion = '9.8.2';
      if (window.electronAPI && window.electronAPI.getAppVersion) {
        // 同步获取版本号（通过 IPC）
        var verInfo = null;
        try { verInfo = window.electronAPI.getAppVersion(); } catch(e) {}
        if (verInfo && verInfo.version) currentVersion = verInfo.version;
      }
