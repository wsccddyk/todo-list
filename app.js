...
`;
  ...
  function handleUpdateEvent(data) {
    ...
    // 【v9.9.12 核心修复】
    // 手动检查期间（_checkingForUpdate=true），所有事件都交给 IPC Promise 的 .then/.catch 处理
    // 绝不能在此处弹窗，否则与手动检查的结果弹窗冲突（用户看到"点了没反应"）
    if (_checkingForUpdate) {
      logInfo('UPDATE', '【忽略事件】手动检查进行中，事件交由IPC回调处理:', data.state);
      return;
    }

    switch (data.state) {
      case 'available':
        // 只有非手动检查时才自动弹窗（如启动后自动发现新版本）
